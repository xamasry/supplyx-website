...
export default function BuyerOrders() {
  ...
  useEffect(() => {
    ...
    const unsubAuth = onAuthStateChanged(auth, (user) => {
      ...
      unsubSnapshot = onSnapshot(qRequests, (reqSnap) => {
        onSnapshot(qOrders, (orderSnap) => {
          const reqData = handleData(reqSnap);
          const orderData = handleData(orderSnap);
          const allData = [...reqData, ...orderData];
  ...
  const activeStatuses = ['accepted', 'preparing', 'shipped', 'pending'];
  ...
  const historyOrders = enhancedRequests.filter(r => r.status === 'delivered' || r.status === 'cancelled' || r.status === 'rejected' || r._isExpired);
